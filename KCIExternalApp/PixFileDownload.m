...
-(void) downloadFileFromUrlInBackgroundTask:(NSMutableArray*)paramArray
{
	...
	if ( response == NO ) {
		...
		[self performSelectorOnMainThread:@selector(downloadComplete:)  
							   withObject:[NSArray arrayWithObjects:newFilePath, fileName, nil] 
                                    waitUntilDone:YES];  	
...
//
-(void) downloadComplete:(NSArray *)info {
    
	//NSLog(@"in PixFileDownload.downloadComplete");	
	NSString * jsCallBack = [NSString stringWithFormat:@"pixFileDownloadComplete('%@', '%@');",[info objectAtIndex:0], [info objectAtIndex:1]];    
